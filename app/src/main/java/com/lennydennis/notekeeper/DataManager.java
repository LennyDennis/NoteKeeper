...
public class DataManager {
    ...
    public static void loadFromDatabase(NoteKeeperOpenHelper noteKeeperOpenHelper) {
        SQLiteDatabase database = noteKeeperOpenHelper.getReadableDatabase();
    ...
    private static void loadCoursesFromDatabase(Cursor courseCursor) {
        int courseIdPosition = courseCursor.getColumnIndex(CourseInfoEntry.COLUMN_COURSE_ID);
        int courseTitlePosition = courseCursor.getColumnIndex(CourseInfoEntry.COLUMN_COURSE_TITLE);

        DataManager dataManager = getInstance();
        dataManager.mCourses.clear();
        while (courseCursor.moveToNext()) {
            String courseId = courseCursor.getString(courseIdPosition);
            String courseTitle = courseCursor.getString(courseTitlePosition);

            CourseInfo courseInfo = new CourseInfo(courseId, courseTitle, null);
            dataManager.mCourses.add(courseInfo);
        }
        courseCursor.close();
    }
